...
LEFT JOIN attack_patterns ap ON e.enemy_id = ap.enemy_id
LEFT JOIN audio_cues ac ON e.enemy_id = ac.enemy_id
WHERE e.name = 'Bubble Head Nurse';
...
FROM vulnerabilities v
JOIN enemies e ON v.enemy_id = e.enemy_id
WHERE v.weak_to LIKE '%Radio Detection%';
...
SELECT e.name, v.resistant_to
FROM vulnerabilities v
JOIN enemies e ON v.enemy_id = e.enemy_id
WHERE v.resistant_to LIKE '%Firearms%';

SELECT e.name, v.resistant_to
FROM vulnerabilities v
JOIN enemies e ON v.enemy_id = e.enemy_id
WHERE v.resistant_to LIKE '%Radio Detection%';

SELECT e.name, v.resistant_to
FROM vulnerabilities v
JOIN enemies e ON v.enemy_id = e.enemy_id
WHERE v.resistant_to LIKE '%Blunt force%';